...
namespace FastKMSWeb.Core.Service
{
    ...
    {
        public PageResult TableList(string key, int pageId, int pageSize)
        {
            ...
            try
            {
                ...
                {
                    sql = $"select table_name as tabName, table_comment as tabComments from information_schema.TABLES where table_schema='{config.Schema}' and table_type='BASE TABLE'";
                ...
                {
                    sql = $"select a.table_name as tabName,comments as tabComments from all_tables a inner join all_tab_comments b on a.TABLE_NAME=b.TABLE_NAME and a.TABLESPACE_NAME='{config.Schema}'";
                    sql = $"select * from(select field.*,ROWNUM RN from({sql}) field where rownum<={pageId * pageSize}) where rn>={(pageId - 1) * pageSize + 1}";
                    sqlCount = $"select count(0) from all_tables a inner join all_tab_comments b on a.TABLE_NAME=b.TABLE_NAME and a.TABLESPACE_NAME='{config.Schema}'";
                }

                if (config.DbType == DataDbType.SqlServer)
                {
                    sql = "select name as tabName,(select top 1 value from sys.extended_properties where major_id=object_id(a.name) and minor_id=0) as tabComments from sys.objects a where type='U'";
        ...
        public List<ColumnInfo> ColumnList(string key, string tableName)
        {
            try
            {
                var config = AppSetting.DataConfig.Find(a => a.Key == key);
        ...
        public bool UpdateColComments(string key, string table, ColumnInfo column)
        {
            try
            {
                var config = AppSetting.DataConfig.Find(a => a.Key == key);
        ...
        public bool UpdateTabComments(string key, TableInfo table)
        {
            try
            {
                var config = AppSetting.DataConfig.Find(a => a.Key == key);
        ...
        public string TableSql(string key, string tableName)
        {
            try
            {
                var columnList = ColumnList(key, tableName);
        ...
        public List<Dictionary<string, object>> NL2Data(string key, string sql, string table)
        {
            var result = new List<Dictionary<string, object>>();
            try
            {
                var config = AppSetting.DataConfig.Find(a => a.Key == key);
                var columnList = ColumnList(key, table);
                using (var conn = DbProviderFactories.GetFactory(key).CreateConnection())
                {
                    conn.ConnectionString = config.ConnStr;
                    conn.Open();
